...
$stmt = $conn->prepare("SELECT coffee_type, quantity, total_price, created_at, name, surname, address FROM orders WHERE client_id = ? ORDER BY created_at DESC");
$stmt->bind_param("i", $client_id);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows > 0): ?>
    <div style="max-width: 1000px; margin: 40px auto; background: white; padding: 30px; border-radius: 10px;">
